...
public class DataLoader<TEntity>(IRepository<TEntity> _repository) : IDataLoader<TEntity> where TEntity : class
{
    ...
    public virtual Task<TEntity?> Get(Expression<Func<TEntity, bool>>? filter = null, bool asNoTracking = true)
        => _repository.Get(filter, asNoTracking);

    public virtual Task<List<TEntity>> List(Expression<Func<TEntity, bool>>? filter = null, bool asNoTracking = true)
