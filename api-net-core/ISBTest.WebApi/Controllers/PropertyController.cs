...
public class PropertyController : ControllerBase
{
    ...
    private readonly IPropertyChangeOwnerProcessor _propertyChangeOwnerProcessor;
    ...
    public PropertyController(
        ...
        IPropertyChangeOwnerProcessor propertyChangeOwnerProcessor)
    {
        _logger = logger;
        _dataLoader = dataLoader;
        _crudProcessor = crudProcessor;
        _propertyChangeOwnerProcessor = propertyChangeOwnerProcessor;
        _mapper = mapper;
    ...
    /// <summary>
    /// To change ownership of a property
    /// </summary>
    /// <param name="propertyId">Propery Id</param>
    /// <param name="changeModel">Payload of the change</param>
    /// <returns></returns>
    [HttpPut("{propertyId}/changeowner")]
    public async Task<IActionResult> ChangeOwner(string propertyId, [FromBody] ChangeOwnerModel changeModel)
    {
        if (!Guid.TryParse(propertyId, out var guid))
            return BadRequest();

        var succeed = await _propertyChangeOwnerProcessor.Change(guid, changeModel.ContactId, changeModel.PurchaseDate, changeModel.AskingPrice, changeModel.SoldPrice);
        if (!succeed)
            return BadRequest();
